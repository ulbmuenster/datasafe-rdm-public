...
export class SearchWithRoleSelection extends Component {
  ...
  render() {
    ...
    const addButtonContent = ((createButtonContent) => {
      if (selectedCount === 0) {
        return i18next.t("Add person");
      }
      if (selectedCount === 1) {
        return i18next.t("Add {{ selectedCount }} person", { selectedCount: selectedCount });
      }
      return i18next.t("Add {{ selectedCount }} persons", { selectedCount: selectedCount });
    })();
        ...
        <Modal.Content className="default-padding">
          ...
          <Form>
            ...
            {searchType === "group" || searchType === "role" ? (
              <i>
                {i18next.t(
                  "Note: upon addition, selected groups will have access to the dataset without any kind of notification.",
                )}
              </i>
            ) : (
